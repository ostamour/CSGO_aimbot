...
void AimBot::updateData()
{
	...
	for (int i = 0; i < nPlayers_; i++)
	{
		if (players_[i] != 0)
		{
			players_[i]->headPosX = memoryManager_->readHeadPosX(i);
			players_[i]->headPosY = memoryManager_->readHeadPosY(i);
			players_[i]->headPosZ = memoryManager_->readHeadPosZ(i);
...
{
	return players_[0]->posX - players_[playerID]->headPosX;
}
...
{
	return players_[0]->posY - players_[playerID]->headPosY;
}
